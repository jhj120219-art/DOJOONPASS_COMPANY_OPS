...
"최소한"이라고 쓴 것이 §27을 구현할 여지를 남긴다. 현재 구현은 여기에
`image_name`(홀더의 실행 파일 이름)을 하나 더 적는다.
...
이유는 §27이 던지는 질문 자체다. PID는 프로세스의 영구한 이름이 아니고 —
Windows는 재부팅 뒤 낮은 PID를 즉시 재배정한다 — Lock 파일은 설계상
프로세스보다 오래 산다(정전, 강제 종료, ExecutionTimeLimit 만료). PID만
가지고는 "**해당** Process가 실행 중인가"가 아니라 "이 PID로 **뭔가**
돌고 있는가"밖에 물을 수 없고, 재사용된 PID 하나가 Lock을 영원히 살아
있는 것처럼 보이게 만든다(BACKLOG C138에 실측이 있다).

이 필드가 없는 Lock — 이 필드 이전에 쓰인 것 — 은 PID만으로 판단한다.
